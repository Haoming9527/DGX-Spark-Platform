...
export function MessageBubble({ message }: MessageBubbleProps) {
  return (
    <motion.div
      ...
      className={`flex gap-2.5 sm:gap-6 w-full ${
        message.role === "user" ? "flex-row-reverse" : "flex-row"
      }`}
    >
      {/* Avatar */}
      <div className="flex-none w-7 sm:w-10">
        {message.role === "assistant" ? (
          <div className="w-7 h-7 sm:w-10 sm:h-10 rounded-full bg-nvidia-green/10 flex items-center justify-center border border-nvidia-green/30 text-nvidia-green shadow-[0_0_10px_rgba(118,185,0,0.15)]">
            <Bot className="w-4 h-4 sm:w-6 sm:h-6" />
          </div>
        ) : (
          <div className="w-7 h-7 sm:w-10 sm:h-10 rounded-full bg-panel flex items-center justify-center border border-border">
            <User className="w-4 h-4 sm:w-6 sm:h-6 text-foreground/40" />
          </div>
        )}
      </div>

      {/* Message Content Container */}
      <div className={`flex flex-col gap-2 min-w-0 max-w-[calc(100%-2.25rem)] sm:max-w-[85%] ${message.role === "user" ? "items-end" : "items-start"}`}>
        ...
        {/* Main Bubble */}
        <div
          className={`w-full max-w-full px-3 sm:px-4 py-2.5 overflow-x-auto rounded-2xl border text-sm sm:text-base ${
            message.role === "user"
        ...
        >
          {message.content ? (
            <div className="prose dark:prose-invert max-w-none prose-sm sm:prose-base prose-pre:p-0 prose-pre:bg-transparent prose-p:break-words">
              <ReactMarkdown
